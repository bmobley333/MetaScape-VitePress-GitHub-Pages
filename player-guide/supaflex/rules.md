...
5. **🔌 Mods vs. 🧿 Gear Powers Canonical Invariants:**
   ...
   * **Gear Powers (`🧿`) = Rules-Engine Execution & Slot Bandwidth Layer:**
     ...
     * *UI Presentation (Exotic Gear Card & Manager):* On the character sheet, the card is titled **"Exotic Gear"** and managed via **"Exotic Gear Manager"**. Gear items render as clean, unified glassmorphic pill buttons showing only the item name and note icon (`ℹ️`), which expand to reveal `📦 Inherent (No Mod)` (no note icon) and installed mods (`🔌 [Mod Name]`, with `ℹ️` only if notes exist). Under each mod with active powers, 1+ left-indented power cards render identically to the `My Powers` card layout (with the left indent absorbed exclusively by narrowing the power name column).
